...
- 🎯 **看板** — 拖拽式票務流轉，跨越 4 個欄位：待做、進行中、受阻、完成
...
2. **帶過的票務**
   - 未完成的票務會自動載入到下一個衝刺的規劃頁面
   - 開始衝刺前可編輯標題/點數

3. **多分頁行為**
   - 每個瀏覽器分頁獨立運作（無同步）
   - 同一天內，多個分頁可能都收到通知（設計如此，以防遺漏提醒）
...
5. **資料備份**
   - 在開發者工具中檢查 localStorage：F12 → Application → Local Storage
   - 匯出為 JSON，如有多個衝刺請保持備份
...
| 問題 | 解決方案 |
|------|--------|
| 通知未推送 | 檢查設定頁面的通知權限；確保分頁保持開啟 |
| 重新整理後資料遺失 | 確認 localStorage 已啟用；檢查開發者工具中 `agile-life-app/v1` 的值 |
| 拖拽功能失效 | 確保使用現代瀏覽器（Chrome、Firefox、Safari、Edge）；試試重新整理 |
| 圖表未顯示 | 確保至少有一個已完成的衝刺（進入 `/review` 並結束衝刺） |
...
本專案為開源專案。可自由用於個人用途和修改。

---

## 🤝 回饋建議

這是個人專案，但歡迎反饋和建議！如果你在使用它，有任何想法都歡迎分享。

---

**用❤️打造，為了一個衝刺接著一個衝刺地管理人生。**
